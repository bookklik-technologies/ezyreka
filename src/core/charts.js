...
export const CHART_PRESETS = [
  { type: 'bar', label: 'Bar', group: 'Bar charts' },
  { type: 'row', label: 'Row', group: 'Bar charts' },
  { type: 'grouped-bar', label: 'Grouped bar', group: 'Bar charts' },
  { type: 'line', label: 'Line', group: 'Line charts' },
  { type: 'multi-line', label: 'Multi-line', group: 'Line charts' },
  { type: 'pie', label: 'Pie', group: 'Pie and donut charts' },
  { type: 'donut', label: 'Donut', group: 'Pie and donut charts' },
  { type: 'area', label: 'Area', group: 'Area charts' },
  { type: 'stacked-area', label: 'Stacked area', group: 'Area charts' }
];
export const isCircularChart = type => type === 'pie' || type === 'donut';
export const isMultiSeriesChart = type => ['grouped-bar', 'multi-line', 'stacked-area'].includes(type);
...
export function normalizeChart(input = {}) {
  ...
  const type = CHART_PRESETS.some(p => p.type === input.type) ? input.type : 'bar';
  ...
  return {
    ...
    series: source.map((s, i) => ({
      name: String(s?.name ?? `Series ${i + 1}`), color: color(s?.color, chartColor(i)),
      values: categories.map((_, r) => Number.isFinite(s?.values?.[r]) ? s.values[r] : null)
    })),
    categoryColors: categories.map((_, i) => color(input.categoryColors?.[i], chartColor(i))),
    ...
    textColor: color(input.textColor, '#374151')
  };
}

export function validateChart(chart) {
  if (isCircularChart(chart.type) && chart.series[0]?.values.some(v => v !== null && v < 0)) {
    throw new Error('Pie and donut charts need non-negative values in the first series.');
  }
  if (chart.type === 'stacked-area' && chartStacks(chart).some(series => series.some(v => v && !Number.isFinite(v.end)))) {
...
export function chartDomain(chart) {
  ...
  if (chart.type === 'stacked-area') chartStacks(chart).forEach(s => s.forEach(v => { if (v) { add(v.start); add(v.end); } }));
